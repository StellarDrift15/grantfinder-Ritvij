...
import EssayDraftTool from "@/components/draft/EssayDraftTool";
    ...
    <Shell active="">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-gf-low">Draft a grant essay, or roleplay a funder to critique your application.</p>
        </div>
      </div>

      <div className="flex gap-1 rounded-xl bg-[rgba(7,11,20,0.55)] border border-gf-line p-1 max-w-md mb-6">
        <button
          type="button"
          onClick={() => setMode("review")}
          className={`flex-1 flex items-center justify-center gap-1.5 rounded-lg px-3 py-2 text-[13px] font-semibold transition-colors ${
            mode === "review" ? "bg-gf-panel-hi text-[#7DD3FC]" : "text-gf-low hover:text-gf-mid"
          }`}
        >
          <ClipboardCheck size={14} /> Review a draft
        </button>
        <button
          type="button"
          onClick={() => setMode("draft")}
          className={`flex-1 flex items-center justify-center gap-1.5 rounded-lg px-3 py-2 text-[13px] font-semibold transition-colors ${
            mode === "draft" ? "bg-gf-panel-hi text-[#A78BFA]" : "text-gf-low hover:text-gf-mid"
          }`}
        >
          <Wand2 size={14} /> Draft an essay
        </button>
      </div>

      {mode === "review" ? <ReviewTool /> : <EssayDraftTool />}
    </Shell>
  );
}